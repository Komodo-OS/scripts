...
if [ "$#" -ne 2 ]; then
    echo "Usage ${0} <branch want backup> <new branch>"
    exit 1
fi
...
cat "${BRANCHLIST}" | while read l; do
    ...
    if git checkout "${OLDBRANCH}" -b "${NEWBRANCH}" > /dev/null
    then
        echo -e "${TOP}/${PROJECTPATH} $grn Success backup ${NEWBRANCH} $end"
    else
